...
export default class Config {
  ...
  async loadConfig(): Promise<boolean> {
    ...
    if (!this.loadedConfig) {
      for (let i = 0; i < configLoader.length; i++) {
        ...
        this.configsStore[configLoader[i].name] = await fetch(module.default).then(res =>
          res.json(),
        )
      }
  ...
  getConfig(name: string) {
    if (typeof this.configsStore[name] === 'undefined') throw new NotFoundConfig('Config not found')
  ...
  getProperty<T>(propertyName: string, defaultValue: T): T {
    const result = propertyName
      .split('.')
      .reduce(
        (acc: unknown, part: string) => acc && (acc as Record<string, unknown>)[part],
        this.configsStore,
      ) as T
